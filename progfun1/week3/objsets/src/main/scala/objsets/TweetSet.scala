...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  /*
   * Obviously my filter could be faster since this implementation:
   ...
   *
   */
  def mostRetweeted: Tweet = remove(elem).mostRetweetedAcc(elem)

  def mostRetweetedAcc(tweet: Tweet): Tweet =
    if (elem.retweets > tweet.retweets) remove(elem).mostRetweetedAcc(elem)
    else remove(elem).mostRetweetedAcc(tweet)
...
object GoogleVsApple {
  ...
  def findTweets(l: List[String], acc: TweetSet): TweetSet =
    if (l.isEmpty) acc
    else findTweets(l.tail,acc.union(tweets.filter(tw => tw.text.toUpperCase.contains(l.head.toUpperCase))))


  lazy val googleTweets: TweetSet = findTweets(google, new Empty)
  lazy val appleTweets: TweetSet = findTweets(apple, new Empty)
